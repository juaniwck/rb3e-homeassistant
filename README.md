...
- **WLED engine (optional)** — a second, independent light engine that streams **per-LED frames straight to a WLED device** over its UDP realtime protocol (DNRGB) at up to 50 fps, with the full 4-color × 8-position Stage Kit state mapped across the strip, real 6–12 Hz strobe, and automatic hand-back to WLED/Hyperion when the song ends. Run both engines at once: slow lights (Hue) on the entity engine, addressable strips on the WLED engine.
...
The integration has two independent Stage Kit light engines. Enable either or both from the integration's **Configure** menu:

| | Smart light engine | WLED engine |
|---|---|---|
| Talks to | Home Assistant `light` entities (service calls) | A WLED device directly (UDP realtime, no entities) |
| Best for | Slower lights: Hue, LIFX, Zigbee bulbs | Addressable strips running WLED |
| Granularity | Per entity (or per WLED *segment* entity) | **Per LED** |
| Update rate | Rate-limited (default 100 ms) | Up to 50 fps (default 20 ms) |
| Strobe | Approximated (~1–3 Hz toggling) | Real Stage Kit rates (6–12 Hz) |

### WLED engine

The WLED engine replaces a dedicated ESP32 Stage Kit build (e.g. ESP32-S2-Mini-Stage-Kit): flash the ESP with stock WLED instead, and this engine renders the Stage Kit cues onto it — while WLED remains a normal ambient/Hyperion light the rest of the time.

**Setup:** Configure → **WLED engine** → enable it, enter the WLED host/IP, LED count, and (optionally) a start offset to drive only part of the strip. Make sure *Receive UDP realtime* is enabled in WLED (Sync settings — it's on by default).

**Mapping.** The Stage Kit state is 4 color rings × 8 LED positions. Two built-in layouts:

- **Blend** (default): the strip is split into 8 position segments; simultaneous colors in a segment blend additively (red + green = yellow). Chase cues sweep along the strip.
- **Interleave**: each of the 8 segments is further split into red/green/blue/yellow lanes, so simultaneous colors render side by side like the physical pod.

For full control (ESP32-SK style), paste a **custom map** — JSON of color → position (`"0"`–`"7"`) → strip LED indices, plus an optional `"strobe"` list:

```json
{
  "red":    {"0": [0, 1], "1": [2, 3], "2": [4, 5]},
  "green":  {"0": [6, 7]},
  "blue":   {"0": [8, 9]},
  "yellow": {"0": [10, 11]},
  "strobe": [0, 1, 2, 3, 4, 5]
...
Positions not listed simply don't light. Indices are absolute strip positions.

**Hyperion / ambient coexistence.** Every frame carries WLED's realtime timeout (default 2 s). When a song ends, you return to the menus, or the game sends *disable all*, the engine pushes one black frame with a 1-second timeout and goes silent — WLED then exits realtime mode and hands the strip back to its own effects or to Hyperion automatically. No switching automations needed. (If Hyperion streams to WLED continuously, the two sources will fight mid-song; have Hyperion idle/black out while Rock Band is playing, e.g. via an automation on `binary_sensor.rock_band_3_enhanced_in_song`.)

**Strobe** flashes the strip white at the real Stage Kit rates (~6/8/10/12 Hz), restoring the LED colors between flashes. Disable it per-strip if you'd rather keep strobe on the entity engine.

**Fog** stays on the smart light engine (map a smart plug there); it isn't a strip effect.
...
- **Color channels (red / green / blue / yellow, "mixed"):** whole-ring lights. Each mapped light represents that whole color ring; assign the same light to several channels and the active colors **blend** (weighted by how many LEDs are lit in each ring). Great for room bulbs.
- **Positional zones (red / green / blue / yellow, "ordered"):** replicates the pod's 8 LED positions per color, like ESP32-S2-Mini-Stage-Kit's 4×8 groups. Add entities **in physical order**; each color ring's 8 LED slots are split contiguously across them — 8 entities = 1 LED each (full positional fidelity), 4 = 2 each, 2 = left/right halves, 1 = whole ring. An entity's brightness follows the fraction of *its own* slots that are lit, and an entity listed under several colors blends whatever is lit at its position, so venue chase/sweep patterns render spatially like on the real pod.

### Replacing an ESP32-S2-Mini-Stage-Kit with WLED

1. Flash WLED on the ESP32 and carve the strip into **8 segments** in WLED's segment editor.
2. Add the WLED integration in Home Assistant — each segment becomes its own `light.` entity.
3. In this integration's options, add the 8 segment entities **in physical order** to *Red positional zones*, then the same 8 in the same order to *Green*, *Blue*, and *Yellow*. Every position now shows the live blend of all four rings at that spot.
4. Set the minimum update interval to ~50 ms, and either point *Strobe lights* at the WLED master entity or use an `rb3e_event` automation to fire WLED's native Strobe effect (a true fast strobe).
